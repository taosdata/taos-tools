...
##### taosBenchmark can test TDengine's insertion, query and subscription functionalities.

##### taosBenchmark can simulate massive data generated by a large number of devices and also flexibly control the number of columns, data types, threads and so on.

##### taosBenchmark support two kinds of configuration, one is command line and the other is json file.

##### formerly called taosdemo, now renamed to taosBenchmark, and the installation package provides taosdemo as a soft link to taosBenchmark.

### taosBenchmark Command line options：

-c：The directory where the config file in, the default path is /etc/taos/

-h：FQDN used to connect taosd server, the default value is localhost

-P：port number used to connect taosd server, the default value is 6030

-u：user name used to connect taosd server, the default value is root

-p：password used to connect taosd server, the default value is taosdata

-I：how taosBenchmark insert data, the options are taosc, rest, stmt, sml, the default value is taosc

-o：specify the path of the result output file, the default value is ./output.txt

-T：specify the number of thread to insert data, the default value is 8

-i：the insert interval for interlace insert mode, unit is ms, default value is 0

-S：insert timestamp step for each record in each sub-table, unit is ms, the default value is 1

-B：the number of interlaced row insert into sub-tables, if the value is 0, the sub-tables are inserted in progressive, the default value is 0

-r：the number of records in each insert request, which is also the batch size, the default value is 30000

-t：the number of sub-tables, the default value is 10000

-n：the number of record inserted for each sub-tables, the default value is 10000

-d：the name of the database, the default value is test

-l：the number of columns in sub-tables, will use columns with int data type.

-A: the data type of sub-tables' tags, nchar and binary can specify the length, such as int,nchar\(18\), the default value is int,binary 

-b：the data type of sub-tables' columns, nchar and binary can specify the length, such as int,nchar\(18\), the default value is float,int,float

-w：the default length of nchar and binary data type, the default value is 64

-m：the prefix of sub-tables' name, the default value is d

-E：whether to use escape character in stable and sub-table name, optional.

-C：whether nchar and binary are basic unicode chinese characters, optional.

-N：whether create super table, optional

-M：whether the data source is random or simulated, optional

-x：whether query partial aggregation function after insertion, optional

-y：whether to pass press Enter to continue, optional

-O：the ratio of inserting data with disorder timestamp, the default is 0

-R：if -O is configured, specify the range of disorder timestamp, based on database's precision, the default value is 1000

-a：the number of replica when create the database, the default value is 1

-V：print taosBenchmark version info

--help：print command line description

### taosBenchmark json configuration file：

##### -f: the json configuration file taosBenchmark read, if set, other command line parameter are invalid

##### taosBenchmark support three type of json configuration files：insert, query and subscribe
