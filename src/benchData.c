...
int stmt_prepare(SSuperTable *stbInfo, TAOS_STMT *stmt, uint64_t tableSeq) {
  ...
  if (stbInfo->autoCreateTable) {
    len += sprintf(prepare + len, "INSERT INTO ? USING `%s` TAGS (%s) VALUES(?", stbInfo->stbName,
...
static int generateSampleFromCsvForStb(char *buffer, char *file, int32_t length, int64_t size) {
  size_t  n = 0;
  ssize_t readLen = 0;
  char *  line = NULL;
  int     getRows = 0;
...
static void calcRowLen(SSuperTable *stbInfo) {
  ...
  for (int colIndex = 0; colIndex < stbInfo->columnCount; colIndex++) {
    ...
    if (stbInfo->iface == SML_IFACE || stbInfo->iface == SML_REST_IFACE) {
      stbInfo->lenOfCols += SML_LINE_SQL_SYNTAX_OFFSET + strlen(stbInfo->columns[colIndex].name);
    }
  }
  stbInfo->lenOfCols += TIMESTAMP_BUFF_LEN;
  ...
  for (int tagIndex = 0; tagIndex < stbInfo->tagCount; tagIndex++) {
    switch (stbInfo->tags[tagIndex].type) {
      ...
      case TSDB_DATA_TYPE_JSON:
        stbInfo->lenOfTags += (JSON_BUFF_LEN + stbInfo->tags[tagIndex].length) * stbInfo->tagCount;
        return;
    }
    stbInfo->lenOfTags += 1;
    if (stbInfo->iface == SML_IFACE || stbInfo->iface == SML_REST_IFACE) {
      stbInfo->lenOfTags += SML_LINE_SQL_SYNTAX_OFFSET + strlen(stbInfo->tags[tagIndex].name);
    }
  }

  if (stbInfo->iface == SML_IFACE || stbInfo->iface == SML_REST_IFACE) {
    stbInfo->lenOfTags += 2 * TSDB_TABLE_NAME_LEN * 2 + SML_LINE_SQL_SYNTAX_OFFSET;
  }
  return;
}

void generateRandData(SSuperTable *stbInfo, char *sampleDataBuf, int lenOfOneRow, Column *columns, int count, int loop,
                      bool tag) {
  ...
  if (iface == STMT_IFACE) {
    for (int i = 0; i < count; ++i) {
      if (columns[i].type == TSDB_DATA_TYPE_BINARY || columns[i].type == TSDB_DATA_TYPE_NCHAR) {
  ...
  for (int k = 0; k < loop; ++k) {
    pos = k * lenOfOneRow;
    if (line_protocol == TSDB_SML_LINE_PROTOCOL && (iface == SML_IFACE || iface == SML_REST_IFACE) && tag) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      switch (columns[i].type) {
        case TSDB_DATA_TYPE_BOOL: {
          ...
          if (iface == STMT_IFACE) {
            *(bool *)(columns[i].data + k * columns[i].length) = rand_bool;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%s,", columns[i].name, rand_bool ? "true" : "false");
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%s ", columns[i].name, rand_bool ? "true" : "false");
            } else {
              pos += sprintf(sampleDataBuf + pos, "%s ", rand_bool ? "true" : "false");
            }
          } else {
            pos += sprintf(sampleDataBuf + pos, "%s,", rand_bool ? "true" : "false");
          }
        ...
        case TSDB_DATA_TYPE_TINYINT: {
          ...
          int8_t tinyint = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          if (iface == STMT_IFACE) {
            *(int8_t *)(columns[i].data + k * columns[i].length) = tinyint;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%di8,", columns[i].name, tinyint);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%di8 ", columns[i].name, tinyint);
            } else {
              pos += sprintf(sampleDataBuf + pos, "%di8 ", tinyint);
            }
        ...
        case TSDB_DATA_TYPE_UTINYINT: {
          ...
          uint8_t utinyint = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          if (iface == STMT_IFACE) {
            *(uint8_t *)(columns[i].data + k * columns[i].length) = utinyint;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%uu8,", columns[i].name, utinyint);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%uu8 ", columns[i].name, utinyint);
            } else {
        ...
        case TSDB_DATA_TYPE_SMALLINT: {
          ...
          int16_t smallint = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          if (iface == STMT_IFACE) {
            *(int16_t *)(columns[i].data + k * columns[i].length) = smallint;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%di16,", columns[i].name, smallint);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%di16 ", columns[i].name, smallint);
            } else {
              pos += sprintf(sampleDataBuf + pos, "%di16 ", smallint);
            }
        ...
        case TSDB_DATA_TYPE_USMALLINT: {
          ...
          uint16_t usmallint = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          if (iface == STMT_IFACE) {
            *(uint16_t *)(columns[i].data + k * columns[i].length) = usmallint;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%uu16,", columns[i].name, usmallint);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%uu16 ", columns[i].name, usmallint);
            } else {
              pos += sprintf(sampleDataBuf + pos, "%uu16 ", usmallint);
            }
        ...
        case TSDB_DATA_TYPE_INT: {
          ...
          if ((g_arguments->demo_mode) && (i == 0)) {
            ...
            int_ = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          }
          if (iface == STMT_IFACE) {
            *(int32_t *)(columns[i].data + k * columns[i].length) = int_;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%di32,", columns[i].name, int_);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%di32 ", columns[i].name, int_);
            } else {
        ...
        case TSDB_DATA_TYPE_BIGINT: {
          ...
          int_ = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          if (iface == STMT_IFACE) {
            *(int64_t *)(columns[i].data + k * columns[i].length) = (int64_t)int_;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%di64,", columns[i].name, int_);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%di64 ", columns[i].name, int_);
            } else {
        ...
        case TSDB_DATA_TYPE_UINT: {
          ...
          uint32_t uint = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          if (iface == STMT_IFACE) {
            *(uint32_t *)(columns[i].data + k * columns[i].length) = uint;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%uu32,", columns[i].name, uint);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%uu32 ", columns[i].name, uint);
            } else {
        ...
        case TSDB_DATA_TYPE_TIMESTAMP: {
          ...
          uint32_t ubigint = columns[i].min + (taosRandom() % (columns[i].max - columns[i].min));
          if (iface == STMT_IFACE) {
            *(uint64_t *)(columns[i].data + k * columns[i].length) = (uint64_t)ubigint;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) &&
              (line_protocol == TSDB_SML_LINE_PROTOCOL || (tag && line_protocol == TSDB_SML_TELNET_PROTOCOL))) {
            pos += sprintf(sampleDataBuf + pos, "%s=%uu64,", columns[i].name, ubigint);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%uu64 ", columns[i].name, ubigint);
            } else {
              pos += sprintf(sampleDataBuf + pos, "%uu64 ", ubigint);
            }
        ...
        case TSDB_DATA_TYPE_FLOAT: {
          ...
          float float_ = (float)(columns[i].min + (taosRandom() % (columns[i].max - columns[i].min)) +
                                 (taosRandom() % 1000) / 1000.0);
          if (g_arguments->demo_mode && i == 0) {
            float_ = (float)(9.8 + 0.04 * (taosRandom() % 10) + float_ / 1000000000);
          } else if (g_arguments->demo_mode && i == 2) {
            float_ = (float)((115 + taosRandom() % 10 + float_ / 1000000000) / 360);
          }
          if (iface == STMT_IFACE) {
            *(float *)(columns[i].data + k * columns[i].length) = float_;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%ff32,", columns[i].name, float_);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%ff32 ", columns[i].name, float_);
            } else {
              pos += sprintf(sampleDataBuf + pos, "%ff32 ", float_);
            }
        ...
        case TSDB_DATA_TYPE_DOUBLE: {
          ...
          double double_ = (double)(columns[i].min + (taosRandom() % (columns[i].max - columns[i].min)) +
                                    taosRandom() % 1000000 / 1000000.0);
          if (iface == STMT_IFACE) {
            *(double *)(columns[i].data + k * columns[i].length) = double_;
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=%ff64,", columns[i].name, double_);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && line_protocol == TSDB_SML_TELNET_PROTOCOL) {
            if (tag) {
              pos += sprintf(sampleDataBuf + pos, "%s=%ff64 ", columns[i].name, double_);
            } else {
              pos += sprintf(sampleDataBuf + pos, "%ff64 ", double_);
            }
        ...
        case TSDB_DATA_TYPE_NCHAR: {
          ...
          if (g_arguments->demo_mode) {
            ...
            cJSON *buf = cJSON_GetArrayItem(columns[i].values, taosRandom() % cJSON_GetArraySize(columns[i].values));
            sprintf(tmp, "%s", buf->valuestring);
          } else {
            rand_string(tmp, columns[i].length, g_arguments->chinese);
          }
          if (iface == STMT_IFACE) {
            sprintf((char *)columns[i].data + k * columns[i].length, "%s", tmp);
          }
          if ((iface == SML_IFACE || iface == SML_REST_IFACE) && columns[i].type == TSDB_DATA_TYPE_BINARY &&
              line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=\"%s\",", columns[i].name, tmp);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && columns[i].type == TSDB_DATA_TYPE_NCHAR &&
                     line_protocol == TSDB_SML_LINE_PROTOCOL) {
            pos += sprintf(sampleDataBuf + pos, "%s=L\"%s\",", columns[i].name, tmp);
          } else if ((iface == SML_IFACE || iface == SML_REST_IFACE) && columns[i].type == TSDB_DATA_TYPE_BINARY &&
            ...
            if (tag) {
              ...
              pos += sprintf(sampleDataBuf + pos, "L\"%s\" ", tmp);
            }
        ...
        case TSDB_DATA_TYPE_JSON: {
          ...
          for (int j = 0; j < count; ++j) {
            ...
            rand_string(buf, columns[j].length, g_arguments->chinese);
...
int prepare_sample_data(int db_index, int stb_index) {
  ...
  calcRowLen(stbInfo);
  if (stbInfo->partialColumnNum != 0 && (stbInfo->iface == TAOSC_IFACE || stbInfo->iface == REST_IFACE)) {
    if (stbInfo->partialColumnNum > stbInfo->columnCount) {
      ...
      for (int i = 0; i < stbInfo->partialColumnNum; ++i) {
        pos += sprintf(stbInfo->partialColumnNameBuf + pos, ",%s", stbInfo->columns[i].name);
      }
      for (int i = stbInfo->partialColumnNum; i < stbInfo->columnCount; ++i) {
  ...
  stbInfo->sampleDataBuf = calloc(1, stbInfo->lenOfCols * g_arguments->prepared_rand);
  infoPrint(stdout,
  ...
  if (stbInfo->random_data_source) {
    generateRandData(stbInfo, stbInfo->sampleDataBuf, stbInfo->lenOfCols, stbInfo->columns, stbInfo->columnCount,
                     g_arguments->prepared_rand, false);
  } else {
    if (stbInfo->useSampleTs) {
      if (getAndSetRowsFromCsvFile(stbInfo)) return -1;
    }
    if (generateSampleFromCsvForStb(stbInfo->sampleDataBuf, stbInfo->sampleFile, stbInfo->lenOfCols,
  ...
  if (!stbInfo->childTblExists && stbInfo->tagCount != 0) {
    stbInfo->tagDataBuf = calloc(1, stbInfo->childTblCount * stbInfo->lenOfTags);
    infoPrint(stdout,
    ...
    if (stbInfo->tagsFile[0] != 0) {
      if (generateSampleFromCsvForStb(stbInfo->tagDataBuf, stbInfo->tagsFile, stbInfo->lenOfTags,
                                      stbInfo->childTblCount)) {
        return -1;
      }
    } else {
      generateRandData(stbInfo, stbInfo->tagDataBuf, stbInfo->lenOfTags, stbInfo->tags, stbInfo->tagCount,
  ...
  if (stbInfo->iface == REST_IFACE || stbInfo->iface == SML_REST_IFACE) {
    if (stbInfo->tcpTransfer && stbInfo->iface == SML_REST_IFACE) {
      if (convertHostToServAddr(g_arguments->host, g_arguments->telnet_tcp_port, &(g_arguments->serv_addr))) {
        errorPrint(stderr, "%s\n", "convert host to server address");
        return -1;
      }
    } else {
      if (convertHostToServAddr(g_arguments->host, g_arguments->port + TSDB_PORT_HTTP, &(g_arguments->serv_addr))) {
        errorPrint(stderr, "%s\n", "convert host to server address");
        return -1;
      }
    }
  ...
  switch (stbInfo->iface) {
    ...
    case TAOSC_IFACE:
      if (stbInfo->autoCreateTable) {
        g_memoryUsage +=
            g_arguments->nthreads * ((stbInfo->lenOfCols + stbInfo->lenOfTags) * g_arguments->reqPerReq + 1024);
      } else {
        g_memoryUsage += g_arguments->nthreads * (stbInfo->lenOfCols * g_arguments->reqPerReq + 1024);
      }
      break;
    case SML_REST_IFACE:
      g_memoryUsage += (stbInfo->lenOfCols + stbInfo->lenOfTags + 1) * g_arguments->reqPerReq * g_arguments->nthreads;
    case SML_IFACE:
      g_memoryUsage += stbInfo->childTblCount * sizeof(char *);
      g_memoryUsage += stbInfo->childTblCount * stbInfo->lenOfTags;
      g_memoryUsage += g_arguments->reqPerReq * sizeof(char *);
      g_memoryUsage += g_arguments->reqPerReq * (stbInfo->lenOfTags + stbInfo->lenOfCols);
      break;
    default:
      break;
  }
...
int bindParamBatch(threadInfo *pThreadInfo, uint32_t batch, int64_t startTime) {
  ...
  for (int c = 0; c < columnCount + 1; c++) {
    ...
    if (c == 0) {
      ...
      debugPrint(stdout, "col[%d]: type: %s, len: %d\n", c, taos_convert_datatype_to_string(data_type),
  ...
  for (uint32_t k = 0; k < batch; k++) {
    ...
    if (stbInfo->disorderRatio) {
      *(pThreadInfo->bind_ts_array + k) =
          startTime + getTSRandTail(stbInfo->timestamp_step, k, stbInfo->disorderRatio, stbInfo->disorderRange);
    } else {
      *(pThreadInfo->bind_ts_array + k) = startTime + stbInfo->timestamp_step * k;
    }
  }
...
int32_t generateSmlJsonTags(cJSON *tagsList, SSuperTable *stbInfo, uint64_t start_table_from, int tbSeq) {
  int32_t code = -1;
  ...
  for (int i = 0; i < stbInfo->tagCount; i++) {
    ...
    switch (stbInfo->tags[i].type) {
      ...
      case TSDB_DATA_TYPE_TINYINT: {
        cJSON_AddNumberToObject(tag, "value", columns[i].min + (taosRandom() % (columns[i].max - columns[i].min)));
      ...
      case TSDB_DATA_TYPE_BIGINT: {
        cJSON_AddNumberToObject(tag, "value",
                                (double)columns[i].min + (taosRandom() % (columns[i].max - columns[i].min)));
      ...
      case TSDB_DATA_TYPE_FLOAT: {
        cJSON_AddNumberToObject(tag, "value",
                                (float)(columns[i].min + (taosRandom() % (columns[i].max - columns[i].min)) +
                                        taosRandom() % 1000 / 1000.0));
      ...
      default:
        errorPrint(stderr, "unknown data type (%d) for schemaless json protocol\n", stbInfo->tags[i].type);
...
int32_t generateSmlJsonCols(cJSON *array, cJSON *tag, SSuperTable *stbInfo, uint32_t time_precision,
                            int64_t timestamp) {
  ...
  switch (stbInfo->columns[0].type) {
    case TSDB_DATA_TYPE_BOOL:
      ...
      break;
    case TSDB_DATA_TYPE_TINYINT:
      cJSON_AddNumberToObject(value, "value", columns[0].min + (taosRandom() % (columns[0].max - columns[0].min)));
    ...
    case TSDB_DATA_TYPE_BIGINT:
      cJSON_AddNumberToObject(value, "value",
                              (double)columns[0].min + (taosRandom() % (columns[0].max - columns[0].min)));
      cJSON_AddStringToObject(value, "type", "bigint");
      break;
    case TSDB_DATA_TYPE_FLOAT:
      cJSON_AddNumberToObject(
          value, "value",
          (float)(columns[0].min + (taosRandom() % (columns[0].max - columns[0].min)) + taosRandom() % 1000 / 1000.0));
