...
void printVersion() {
    ...
    char taosbenchmark_ver[] = taosbenchmark_COMMIT_SHA1;
    char taosbenchmark_status[] = taosbenchmark_STATUS;

    if (strlen(taosbenchmark_status) == 0) {
        printf("taosBenchmark version %s-%s\n", tdengine_ver,
               taosbenchmark_ver);
    } else {
        printf("taosBenchmark version %s-%s, status:%s\n", tdengine_ver,
               taosbenchmark_ver, taosbenchmark_status);
    }
    exit(EXIT_SUCCESS);
}
...
void printHelp() {
    ...
    printf("%s%s%s%s\n", indent, "-I, --interface=INTERFACE", "\t",
           "The interface (taosc, rest, stmt, and sml(line protocol)) "
           "taosbenchmark "
           "uses. By default "
           "use 'taosc'.");
