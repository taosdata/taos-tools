...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
  ...
  SDataBase* db = benchArrayGet(arguments->db, 0);
  ...
  switch (key) {
    ...
    case 'n':
      ...
      break;
    case 'd':
        tstrncpy(db->dbName, arg, TSDB_DB_NAME_LEN);
        break;
    case 'l':
      arguments->demo_mode = false;
    ...
    case 'A':
        ...
        parse_datatype(arg, stbInfo->tags, true);
        break;
    case 'b':
        arguments->demo_mode = false;
        benchArrayClear(stbInfo->columns);
        parse_datatype(arg, stbInfo->columns, false);
      break;
    case 'w':
    ...
    case 'O':
      ...
      break;
    case 'a':
      db->dbCfg.replica = atoi(arg);
      if (db->dbCfg.replica <= 0) {
        errorPrint(
            stderr,
